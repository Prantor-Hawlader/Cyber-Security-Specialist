 ...
 */
export function calculateBaseScore(params: any): number {
  const {
    scope,
    confidentiality,
    integrity,
  ...
  const isc = calculateImpactSubScore(
    scope,
    confidentiality,
    integrity,
    availability
  );
  const ess = calculateExploitableSubScore(
    attackVector,
    attackComplexity,
    privilegeRequired,
    userInteraction
  ...
  if (isc <= 0) {
    ...
    return parseFloat(
      (Math.ceil(Math.min(isc + ess, 10) * 10) / 10).toFixed(1)
    );
  } else if (scope === Scope.CHANGED) {
    return parseFloat(
      (Math.ceil(Math.min(1.08 * (isc + ess), 10) * 10) / 10).toFixed(1)
    );
  }
 ...
 * @returns
 */
export function getWeight(scope: any, privilegeValue: any) {
  const weight = {
    [Scope.UNCHANGED]: {
      N: 0.85,
      L: 0.62,
  ...
  try {
    return (weight as any)[scope][privilegeValue];
  } catch (err) {
 ...
 * @returns
 */
export function checkSeverity(score: any) {
